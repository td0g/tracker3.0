...
#define DEC_MOTOR_MAX_SPEED 75
#define DEC_PULSE_GUIDE_ARCSEC_PER_SEC 1
#define RA_PULSE_GUIDE_ARCSEC_PER_SEC 90000
//Was 50000
...
unsigned int raPulseRate = RA_PULSE_GUIDE_ARCSEC_PER_SEC / 100;               //RA pulse guide rate (500 = 5.00% sidereal rate)
...
unsigned int decPGrate = 1000000 * DEC_PULSE_GUIDE_ARCSEC_PER_SEC / DEC_STEPS_PER_ARCSEC;  //was 10,000  us / step...        us / step = s / arcsec / step / arcsec * 1000000 us / s =     
...
//Focuser
const byte focusStepPin = 10; //9 or 10 or 11 or 12
...
void loop() { //Times tested on 2017-09-11
  ...
  printDebugInfo(); //8us
  runIndicatorLED();  //8us
  //readController(); //4us, >120us when doing an analog read
  focusMotor();
}
  ...
  */
  ...
  if (micros() > nextStepTime) {   //If timer is up and we are not at end of rod
    ...
    if (!raMotorOn) nextStepTime = micros() + 500;                  //Just keep timer updated
      ...
      if (raSlew(0, 0) < 0) {    //Slewing east
        if (digitalRead(raEndStopPin)) {         //Check endstop, if depressed then set home position and stop
          raStepPosition = 2;
          raMotorOn = 0;
          raSlew(2, 0);
        }
...
void decMotor() {
  ...
  */
  ...
  if (micros() > nextStepTime) {
    ...
    if (decGotoPosition != decStepPosition) nextStepTime += decSlewRate;    //Default delay time for moving & slewing
    else if (decMoveDir != 0) {
      nextStepTime += decSlewRate;
      decGotoPosition += decMoveDir;      //Moving
    }
...
void focusMotor() {
  ...
  if (micros() > stepTimer) {
    ...
    if (focusTargetPos != focusCurrPos) {
      if (sleepTimer) {
        sleepTimer = 0;
        ...
        if (focusCurrPos < focusTargetPos) {      //Moving out
          if (!dirPinHigh){
            digitalWrite(focusDirPin, HIGH);
            dirPinHigh = 1;
          }
          focusCurrPos++;
        }
        else if (focusCurrPos > focusTargetPos) { //Moving in
          if (dirPinHigh){
            digitalWrite(focusDirPin, LOW);
            dirPinHigh = 0;
          }
...
void parseCommand() {   //Serial Commander
  ...
  else if (COMstring == "D") Serial.print("q" + String(currPos(1)) + "/");
  else if (COMstring == "R") Serial.print("q" + String(currPos(0)) + "/");
  ...
  else if (COMstring == "mw") slewToPos(currPos(0) + 40000);
  else if (COMstring == "me") slewToPos(currPos(0) - 40000);
  else if (COMstring == "md") decMoveDir = 0;
  else if (COMstring == "mr") slewToPos(currPos(0));

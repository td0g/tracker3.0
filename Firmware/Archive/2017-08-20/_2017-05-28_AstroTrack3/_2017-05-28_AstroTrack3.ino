...
const unsigned long decStepsPerArcsec = 25;
const byte decDirPin = 7;
const byte decStepPin = 6;
unsigned int decSlewRate = 75;            //DEC slewing speed
...
void raMotor(){
  static unsigned long raNextStepTime;
  static char raDir;
  static char steps;
  ...
  if (digitalRead(decEndStopPin)){
    raStepPosition = 2;
    raSlew(2, 0);
    if (raMoveDir > 0) raMoveDir = 0;
  }

  if (raStepPosition < totalSteps && micros() > raNextStepTime){    //If timer is up and we are not at end of rod
    if (!raMotorOn) raNextStepTime = micros() + 500;                  //Just keep timer updated
    else {
      steps = 1;
      if (raSlew(0,0) < 0 || raMoveDir > 0) {                                               //Slewing east
        raDir = 0;
        steps = raSlewFactor;
        if (raStepPosition == 2) raSlew(2,0);                          //Is at home
        raNextStepTime += raSlewRate;
        int raSlewChange = currMotorStepDelay * raSlewFactor + raSlewRate;
        raSlew(0,raSlewChange);
        raSlew(2,1);
        raStepTimeTotal -= currMotorStepDelay * steps;
      }
      else {                                                            //Not slewing east
        raDir = 1;
        if (raSlew(0,0)|| raMoveDir < 0) {                                                  //Slewing west
          ...
          raNextStepTime += currMotorStepDelay;
...
void decMotor(){
  ...
  if (micros() > nextStepTime){
    ...
    if (decGotoPosition > decStepPosition){
      ...
      dir = 1;
      decStepPosition++;
      digitalWrite(decStepPin, decStepPosition % 2);
    }
